...
double frequency_array[MAX_FREQUENCY] = {.5, .6, .75, .85, .91, .96, 1.0};
...
int file_write_freq()
{
    const char *filename = "energyresult.txt";
    FILE *fp = fopen(filename, "a");
    if (!fp)
    {
        printf("Error: could not open file %s\n", filename);
...
{
    int row = 0;
    char *filename = "Set1/Set_1_m_4_U_50_n_20_tss_90.txt"; // argv[1]
    Readfile(filename);
    ...
    int g = min(deadline, MAX_ROWS) / 20;
    ...
    FILE *fp = fopen("tempresult.txt", "a");
    fprintf(fp, "%s%s", filename, "\n");
    fclose(fp);
    FILE *f = fopen("energyresult.txt", "a");
    fprintf(f, "%s%s", filename, "\n");
    fclose(f);
    ...
    // printf("\n");
    sum_frequency=0;
    for (int i = 0; i < MAX_PC; i++)     //energy calculation 
    {
        sum_frequency+= Capacity[i]*0.1*0.1*0.1+ (g-Capacity[i]);
    }
    sum_frequency/=g;
    file_write_freq();
    ...
    // fprintf(fp, "%s%s","\tTEMPERATURE DETAILS\n\n",filename,"\n");
    // fclose(fp);
    ...
    for (int i = 0; i < MAX_PC; i++)
    {
        ...
        sum_frequency+=frequency*frequency*frequency;
