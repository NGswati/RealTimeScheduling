...
int avg_temp[MAX_PC], peak_temp[MAX_PC], end_temp[MAX_PC];
...
void avg_peak_temperature(struct Node *Allocated_seq[MAX_PC][MAX_ROWS], int nor_task_Core[MAX_PC])
{
    for (int i = 0; i < MAX_PC; i++) 
    {
        int temp_array[MAX_ROWS]={0},core_temp=25,sum=0;
        for (int j = 0; j < nor_task_Core[i]; j++)
        {
            if(!Allocated_seq[i][j]){
                temp_array[j] = temp_counter(core_temp, 25, Allocated_seq[i][j]->share_id);
                core_temp = temp_counter(core_temp,25, Allocated_seq[i][j]->share_id);
            }
            else{
                temp_array[j]=temp_counter(core_temp,Allocated_seq[i][j]->tss,Allocated_seq[i][j]->share_id);
                core_temp = temp_counter(core_temp, Allocated_seq[i][j]->tss, Allocated_seq[i][j]->share_id);
            }
        }
        end_temp[i] = temp_array[nor_task_Core[i] - 1];
        for (int j = 0; j < nor_task_Core[i];j++){
            sum+=temp_array[j];
        ...
        peak_temp[i] = max(temp_array, nor_task_Core[i]);
    }
...
int main()
{
    ...
    for(int i=0;i<4;i++){
        for(int j=0;j<nor_task_Core[i];j++){
            printf("%d     %d\n", Allocation_seq[i][j]->share_id, Allocation_seq[i][j]->task_id);
        }
    }

    avg_peak_temperature(Allocation_seq, nor_task_Core);
    printf("Pc_id  End_temp  peak_temp  Avg_temp\n");
    for(int i=0;i<MAX_PC;i++){
