...
int share_func(int et[MAX_ROWS], int sm[MAX_ROWS], int dl[MAX_ROWS], int dlc[MAX_ROWS])
{
    int g = min(dlc, MAX_ROWS);
...
int main()
{
    ...
    //scheduling
    int g = 100000,x=0;
    const int m=4; // min(deadline ,MAX_ROWS)
    int Capacity[4], Allocation_seq[4][MAX_ROWS] = {0}, task_allocation_flag[MAX_ROWS] = {0};
    int no_task_Core[m]={0};
    for(int i=0;i<4;i++)
    {
        Capacity[i]=g;
    }
    ...
    while (pt != NULL)
    {
        ...
        if (pt != NULL)
        {
            Allocation_seq[pt->pc_id][no_task_Core[pt->pc_id]++] = pt->task_id;
    ...
    // while (pt != NULL)
    // {
    //     printf("task_id: %d, share_id: %d, pc_id: %d, tss: %d\n", pt->task_id, pt->share_id, pt->pc_id, pt->tss);
    //     pt = pt->next;
    // }
    for (int i = 0; i < 4; i++)
    {
